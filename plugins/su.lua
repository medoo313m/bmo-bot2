...
--local user_num =   "🚸عدد العضاء: "..result.participants_count.."\n"
--local kicked_num = "🚨عدد الاعضاء المطرودين: "..result.kicked_count.."🏌\n"
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return 'تــم ضــع القوانــين فيـ المجمــوعه ✅'
end
...
local function get_rules(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return 'لــا توجد قوانيــن فيـ هذه المجمــوعه ❎' 
  end
		...
		if matches[1] == 'k' and is_owner(msg) then
			...
			local msg_type = ''
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return msg_type.."تــم قفــل البــصماآت 🔒"
				else
					return "SuperGroup mut"..msg_type.." is already on"
				end
			end
			if matches[2] == '' then
			local msg_type = 'PhotoPhoto'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.." has been mutedmuted"
				else

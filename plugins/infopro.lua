...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = text..'❣ الاسم الثاني : '..(msg.from.last_name or '----')..'\n'	
   local text = text..'❣ المعرف : '..Username..'\n'
   local text = text..'❣ رقم الهاتف : '..(msg.from.phone or 'no')..'\n'	
	...
	if hash then
	  ...
	  if not value then
		if msg.from.id == tonumber(Arian) then
		 text = text..'❣ الرتبه : ادمن جديد \n\n'
		elseif is_sudo(msg) then
		 text = text..'❣ الرتبه : بطور \n\n'
		elseif is_owner(msg) then
		 text = text..'❣ الرتبه : مدير \n\n'
		elseif is_momod(msg) then
		 text = text..'❣ الرتبه : ادمن \n\n'
		else
		 text = text..'❣ الرتبه : عضو \n\n'
		end
	  else
	   text = text..'❣ Rank : '..value..'\n'
	  end
	 ...
	 user_info_msgs = tonumber(redis:get(um_hash) or 0)
	 text = text..'❣ عدد رسائل العضو : '..user_info_msgs..'\n\n'
    if msg.to.type == 'chat' then
